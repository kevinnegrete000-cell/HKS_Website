...
async function initAtlasView(){
  ...
  const surface=new THREE.Mesh(
    ...
    new THREE.MeshPhysicalMaterial({color:0x101515,roughness:.92,metalness:.06,transparent:true,opacity:.7,side:THREE.FrontSide})
  );
  globeGroup.add(surface);
  const atmosphere=new THREE.Mesh(
    new THREE.SphereGeometry(radius*1.035,64,40),
    new THREE.MeshBasicMaterial({color:0x9bb3c1,transparent:true,opacity:.045,side:THREE.BackSide,depthWrite:false})
  );
  globeGroup.add(atmosphere);

  const gridMaterial=new THREE.LineBasicMaterial({color:0xaeb9b7,transparent:true,opacity:.105,depthWrite:false});
  for(let lat=-75;lat<=75;lat+=15){
    const points=[];
    for(let lon=-180;lon<180;lon+=4)points.push(latLonToVector3(lat,lon,radius+.012));
    globeGroup.add(makeAtlasLine(points,gridMaterial,true));
  }
  for(let lon=-165;lon<180;lon+=15){
    const points=[];
    for(let lat=-88;lat<=88;lat+=3)points.push(latLonToVector3(lat,lon,radius+.014));
    globeGroup.add(makeAtlasLine(points,gridMaterial,false));
  }
  const continentMaterial=new THREE.LineBasicMaterial({color:0xe0e3dc,transparent:true,opacity:.56,depthWrite:false});
  const accurateCoastlinesLoaded=await addAccurateAtlasCoastlines(globeGroup,radius,continentMaterial);
  if(!accurateCoastlinesLoaded){
    atlasContinentOutlines().forEach(outline=>{
      const points=outline.map(([lat,lon])=>latLonToVector3(lat,lon,radius+.025));
      globeGroup.add(makeAtlasLine(points,continentMaterial,false));
    });
  }
  ...
  const resizeObserver=new ResizeObserver(resize);resizeObserver.observe(host);resize();
  atlasViewer={host,scene,camera,renderer,controls,globeGroup,markers,arcs,pickables,resizeObserver,resize,animationId:null,elapsed:0,selectedId:null,cameraGoal:null,targetGoal:null,pauseUntil:0,listeners:{onPointerMove,onPointerLeave,onClick,pauseAuto}};
...
console.info('[HKS] Atlas v4 loaded — constellation-to-globe build');
renderObservatory();
renderFilters();
renderRelationships();
renderTimeline();
...
console.info('[HKS] Atlas v4.2 loaded — Natural Earth coastline build');
