...
async function loadAtlasEarthModel(globeGroup,radius){
  ...
  const landWireMaterial=new THREE.MeshBasicMaterial({
    color:0xd5dbd5,
    wireframe:true,
    transparent:true,
    opacity:.065,
  ...
  try{
    ...
    // Mundo.obj contains a raised land shell and a base ocean sphere.
    // Replace the missing source materials with archive-native monochrome ones.
    const earthMeshes=[];
    earth.traverse(object=>{if(object.isMesh)earthMeshes.push(object);});
    earthMeshes.forEach(object=>{
      ...
      object.material=isOcean?oceanMaterial:landMaterial;
      object.castShadow=false;
      object.receiveShadow=false;
      object.geometry?.computeVertexNormals?.();

      if(!isOcean){
        const wire=new THREE.Mesh(object.geometry,landWireMaterial);
        wire.name='atlas-land-wire-overlay';
        wire.renderOrder=object.renderOrder+1;
        wire.scale.setScalar(1.0012);
        object.add(wire);
      }
    });

    // Center and normalize the supplied mesh without assuming its source units.
    ...
    earthRoot.name='mundo-earth-root';
    earthRoot.scale.setScalar(radius/sphere.radius);
    earthRoot.add(earth);
    globeGroup.add(earthRoot);
    return earthRoot;
  }catch(error){
    console.error('[HKS] Mundo.obj could not load; using procedural globe fallback.',error);
    ...
    fallback.name='atlas-procedural-earth-fallback';
    globeGroup.add(fallback);
    return fallback;
  }
}

function makeAtlasArc(sourceItem,targetItem,relationship,radius,index){
  const source=latLonToVector3(sourceItem.atlas.lat,sourceItem.atlas.lon,radius+.035);
  const target=latLonToVector3(targetItem.atlas.lat,targetItem.atlas.lon,radius+.035);
  const angle=source.angleTo(target);
  const height=.25+angle*.62+relationship.strength*.2;
  const points=[];
  for(let step=0;step<=56;step+=1){
    const t=step/56;
    const blended=source.clone().lerp(target,t).normalize();
    blended.multiplyScalar(radius+Math.sin(Math.PI*t)*height+.035);
    points.push(blended);
  }
  const color=relationshipCategoryColor(relationship.category);
  const material=new THREE.LineBasicMaterial({color,transparent:true,opacity:.16,depthWrite:false});
  const line=makeAtlasLine(points,material,false);
  line.userData={relationship,source:relationship.source,target:relationship.target,baseOpacity:.16};
  const signal=new THREE.Mesh(
    new THREE.SphereGeometry(.025,10,8),
    new THREE.MeshBasicMaterial({color,transparent:true,opacity:.7,depthWrite:false})
  );
...
async function initAtlasView(){
  ...
  const radius=2.66;
  const earthModel=await loadAtlasEarthModel(globeGroup,radius);
  if(state.view!=='atlas'||!document.body.contains(host))return;
  ...
  const atmosphere=new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({color:0x9bb3c1,transparent:true,opacity:.052,side:THREE.BackSide,depthWrite:false})
  );
  globeGroup.add(atmosphere);
  const gridMaterial=new THREE.LineBasicMaterial({color:0xb7c0bd,transparent:true,opacity:.082,depthWrite:false});
  ...
  archiveItems.forEach((item,index)=>{
    const group=new THREE.Group();
    const point=latLonToVector3(item.atlas.lat,item.atlas.lon,radius+.018);
    group.position.copy(point);
    group.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),point.clone().normalize());
    const stem=new THREE.Line(
      new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0,0,0),new THREE.Vector3(0,.25,0)]),
      new THREE.LineBasicMaterial({color:0xc9d5d8,transparent:true,opacity:.48})
    );
    const core=new THREE.Mesh(
      new THREE.SphereGeometry(.072,18,14),
      new THREE.MeshStandardMaterial({color:0xe3e5df,emissive:0x6f8796,emissiveIntensity:.58,roughness:.38,transparent:true,opacity:.82})
    );
    core.position.y=.29;core.userData.itemId=item.id;
    const halo=new THREE.Mesh(
      new THREE.TorusGeometry(.135,.008,8,34),
      new THREE.MeshBasicMaterial({color:0xc7d6df,transparent:true,opacity:.24,depthWrite:false})
    );
    halo.rotation.x=Math.PI/2;halo.position.y=.292;
    const label=makeAtlasLabel(item);label.position.set(.16,.54,0);
  ...
  relationships.forEach((relationship,index)=>{
    ...
    const target=archiveItems.find(item=>item.id===relationship.target);
    if(!source||!target)return;
    const arc=makeAtlasArc(source,target,relationship,radius,index);
  ...
  const resizeObserver=new ResizeObserver(resize);resizeObserver.observe(host);resize();
  atlasViewer={host,scene,camera,renderer,controls,globeGroup,earthModel,markers,arcs,pickables,resizeObserver,resize,animationId:null,elapsed:0,selectedId:null,cameraGoal:null,targetGoal:null,pauseUntil:0,listeners:{onPointerMove,onPointerLeave,onClick,pauseAuto}};
...
console.info('[HKS] Atlas v4.4 loaded — Mundo.obj Earth build');
