  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader:`
      ...
      void main(){
        ...
        float edge=smoothstep(.5,.13,distanceFromCenter);
        gl_FragColor=vec4(vec3(1.0),uOpacity*edge*(.82+.18*vPulse));
      }
    `
  });
}
  ...
  try{
    ...
    earth.traverse(object=>{
      ...
      const pointMaterial=createAtlasPointMaterial({
        size:isOcean?1.65:2.45,
        opacity:isOcean?.38:.92
      });
    ...
    // A nearly invisible dark core gives the white points depth while keeping
    // the Earth unmistakably point-cloud rather than a solid shaded globe.
...
console.info('[HKS] Atlas v4.5 loaded — surface-snapped white point-cloud Earth build');
