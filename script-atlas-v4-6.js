...
function createAtlasPointMaterial({size=2.2,opacity=.75}={}){
  return new THREE.ShaderMaterial({
    ...
    vertexShader:`
      ...
      void main(){
        ...
        float pulse=.88+.12*sin(uTime*.7+aPhase);
        vPulse=pulse;
        gl_PointSize=max(1.0,uSize*pulse*(10.0/max(1.0,-mvPosition.z)));
    ...
    fragmentShader:`
      ...
      void main(){
        ...
        // Reversed smoothstep edges are undefined in GLSL and caused the
        // point sprites to disappear on some browsers. Build a proper soft
        // circular point with a bright white center and feathered edge.
        float edge=1.0-smoothstep(.12,.5,distanceFromCenter);
        float centerGlow=1.0-smoothstep(0.0,.2,distanceFromCenter);
        float alpha=max(edge,centerGlow*.42);
        gl_FragColor=vec4(vec3(1.0),uOpacity*alpha*(.88+.12*vPulse));
      }
    `
  });
}
...
function buildAtlasPointGeometry(sourceGeometry,extraSamples=0,seed=1){
...
async function loadAtlasEarthModel(globeGroup,radius){
  const invisibleSurfaceMaterial=new THREE.MeshBasicMaterial({
    transparent:true,
    opacity:0,
    depthWrite:false,
    colorWrite:false,
  ...
  try{
    ...
    earth.traverse(object=>{
      ...
      const isOcean=objectName.includes('azul')||objectName.includes('blinn3sg');
      object.material=invisibleSurfaceMaterial;
      ...
      const pointMaterial=createAtlasPointMaterial({
        size:isOcean?1.55:3.15,
        opacity:isOcean?.30:.98
      });
    ...
    earthRoot.name='mundo-point-cloud-earth-root';
    earthRoot.scale.setScalar(radius/sphere.radius);
    earthRoot.add(earth);

    // A barely-there inner core gives the cloud subtle depth without reading
    // as a solid black sphere. The visible Earth is carried by the white points.
    const depthCore=new THREE.Mesh(
      new THREE.SphereGeometry(radius*.982,64,42),
    ...
    return {root:earthRoot,surfaceMeshes:earthMeshes,pointMaterials,depthCore};
  }catch(error){
    ...
    const fallbackMaterial=createAtlasPointMaterial({size:1.8,opacity:.58});
    ...
    return {root:fallback,surfaceMeshes:[],pointMaterials:[fallbackMaterial],depthCore:null};
  }
}
...
async function initAtlasView(){
  ...
  const gridMaterial=new THREE.LineBasicMaterial({color:0xcbd2cf,transparent:true,opacity:.038,depthWrite:false});
...
console.info('[HKS] Atlas v4.6 loaded — visible white point-cloud Earth build');
