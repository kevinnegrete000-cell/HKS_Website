...
async function initAtlasView(){
  ...
  const animate=now=>{
    ...
    arcs.forEach((arc,index)=>{
      const t=(atlasViewer.elapsed*(.055+index%4*.008)+arc.signal.userData.offset)%1;
      const scaled=t*(arc.points.length-1);const low=Math.floor(scaled);const high=Math.min(arc.points.length-1,low+1);const mix=scaled-low;
      arc.signal.position.copy(arc.points[low]).lerp(arc.points[high],mix);
    });
