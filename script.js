...
function renderObservatory(){const svg=$('#orbitSvg'), layer=$('#nodesLayer');svg.innerHTML='';layer.innerHTML='';
 ...
 archiveItems.forEach((item,index)=>{const node=document.createElement('button');node.className='node';node.dataset.id=item.id;const size=16+item.importance*6;Object.assign(node.style,{left:item.x+'px',top:item.y+'px',width:size+'px',height:size+'px'});node.style.setProperty('--node-speed',`${4.6+(index%7)*.55}s`);node.style.animationDelay=`-${(index%9)*.33}s`;node.setAttribute('aria-label',item.title);const label=document.createElement('span');label.className='node-label';label.textContent=item.title;node.appendChild(label);node.addEventListener('click',e=>{e.stopPropagation();openDetail(item.id)});layer.appendChild(node)});applyTransform()}
