...
const state={view:'observatory',selected:null,scale:.78,x:0,y:0,dragging:false,lastX:0,lastY:0,filters:new Set(['Material','Fabrication','Typology','Geometry','Project Phase']),returnTransform:null};
...
function setView(name){state.view=name;$$('.view').forEach(v=>v.classList.toggle('active-view',v.id===name));$$('.nav-btn').forEach(b=>b.classList.toggle('active',b.dataset.view===name));closeDetail(false);}
...
function renderObservatory(){const svg=$('#orbitSvg'), layer=$('#nodesLayer');svg.innerHTML='';layer.innerHTML='';
 ...
 const gradient=svgEl('linearGradient',{id:'orbitEnergyGradient',x1:'0%',y1:'0%',x2:'100%',y2:'100%'});
 [['0%','rgba(121,154,165,.22)'],['48%','#eefaff'],['68%','#a8d4e3'],['100%','rgba(121,154,165,.18)']].forEach(([offset,color])=>gradient.appendChild(svgEl('stop',{offset,'stop-color':color})));
 const softGlow=svgEl('filter',{id:'softGlow',x:'-60%',y:'-60%',width:'220%',height:'220%'});softGlow.appendChild(svgEl('feGaussianBlur',{stdDeviation:'2.2',result:'blur'}));const softMerge=svgEl('feMerge');softMerge.appendChild(svgEl('feMergeNode',{in:'blur'}));softMerge.appendChild(svgEl('feMergeNode',{in:'SourceGraphic'}));softGlow.appendChild(softMerge);
 const particleGlow=svgEl('filter',{id:'particleGlow',x:'-150%',y:'-150%',width:'400%',height:'400%'});particleGlow.appendChild(svgEl('feGaussianBlur',{stdDeviation:'3',result:'blur'}));const particleMerge=svgEl('feMerge');particleMerge.appendChild(svgEl('feMergeNode',{in:'blur'}));particleMerge.appendChild(svgEl('feMergeNode',{in:'SourceGraphic'}));particleGlow.appendChild(particleMerge);
 defs.append(gradient,softGlow,particleGlow);svg.appendChild(defs);
 relationships.forEach((r,index)=>{const a=archiveItems.find(i=>i.id===r.source),b=archiveItems.find(i=>i.id===r.target);if(!a||!b)return;const pathId=`connection-${index}`;const path=svgEl('path',{id:pathId,d:`M ${a.x} ${a.y} L ${b.x} ${b.y}`,class:'relationship-line'});svg.appendChild(path);if(index%2===0){const particle=svgEl('circle',{r:index%4===0?2.2:1.5,class:'energy-particle'});const motion=svgEl('animateMotion',{dur:`${6+(index%5)}s`,repeatCount:'indefinite',begin:`-${index*.7}s`});motion.appendChild(svgEl('mpath',{href:`#${pathId}`}));particle.appendChild(motion);svg.appendChild(particle)}});
 orbitCenters.forEach((c,centerIndex)=>{for(let i=1;i<=c.ringCount;i++){const rad=c.maxRadius*i/c.ringCount;svg.appendChild(svgEl('circle',{cx:c.x,cy:c.y,r:rad,class:'orbit-base'}));const energy=svgEl('circle',{cx:c.x,cy:c.y,r:rad,class:'orbit-energy'});energy.style.setProperty('--flow-speed',`${10+i*2.3+centerIndex}s`);energy.setAttribute('stroke-dashoffset',String((i+centerIndex)*19));svg.appendChild(energy);const secondary=svgEl('circle',{cx:c.x,cy:c.y,r:rad,class:'orbit-energy secondary'});secondary.style.setProperty('--flow-speed-secondary',`${17+i*3+centerIndex}s`);secondary.setAttribute('stroke-dashoffset',String((i+centerIndex)*-27));svg.appendChild(secondary)}svg.appendChild(svgEl('circle',{cx:c.x,cy:c.y,r:8,class:'orbit-center'}));const tx=svgEl('text',{x:c.x+16,y:c.y-14,class:'orbit-label'});tx.textContent=c.label;svg.appendChild(tx)});
 archiveItems.forEach((item,index)=>{const node=document.createElement('button');node.className='node';node.dataset.id=item.id;const size=16+item.importance*6;Object.assign(node.style,{left:item.x+'px',top:item.y+'px',width:size+'px',height:size+'px'});node.style.setProperty('--node-speed',`${4.6+(index%7)*.55}s`);node.style.animationDelay=`-${(index%9)*.33}s`;node.setAttribute('aria-label',item.title);const label=document.createElement('span');label.className='node-label';label.textContent=item.title;node.appendChild(label);node.addEventListener('click',e=>{e.stopPropagation();focusItem(item.id)});layer.appendChild(node)});applyTransform()}
...
function animateCanvasTo(target,duration=620,onComplete){
  ...
  const tick=now=>{
    const raw=Math.min(1,(now-started)/duration),t=ease(raw);
    ...
    else{canvasAnimationFrame=null;onComplete?.()}
  };
...
function focusItem(id){
  ...
  const focusScale=item.model?1.52:1.28;
  animateCanvasTo({scale:focusScale,x:-item.x*focusScale,y:-item.y*focusScale},640,()=>openDetail(id));
}
...
function openDetail(id){
  ...
  const i=archiveItems.find(x=>x.id===id);
  if(!i)return;
  const panel=$('#detailPanel');
  panel.classList.toggle('model-mode',Boolean(i.model));
  if(i.model){
    $('#detailContent').innerHTML=`<div class="detail-inner model-detail">
      <div class="model-stage-shell">
        <div id="modelViewport" class="model-viewport" aria-label="Interactive 3D model of ${i.title}"></div>
      ...
      <div class="model-information">
        <div class="detail-code">${i.archiveCode}</div>
        <h2>${i.title}</h2>
        <div class="detail-sub">${i.year} · ${i.location}</div>
        <p class="detail-description">${i.introduction}</p>
        <div class="meta">${[['Accession',i.accessionNumber],['Typology',i.typology],['Prototype',i.prototypeType],['Material',i.material],['Fabrication',i.fabricationMethod],['Phase',i.projectPhase]].map(([a,b])=>`<div class="meta-row"><span>${a}</span><span>${b}</span></div>`).join('')}</div>
      </div>
    </div>`;
  }else{
    $('#detailContent').innerHTML=`<div class="detail-inner"><div class="detail-code">${i.archiveCode}</div><h2>${i.title}</h2><div class="detail-sub">${i.year} · ${i.location}</div><div class="image-placeholder"></div><p class="detail-description">${i.introduction}</p><div class="meta">${[['Accession',i.accessionNumber],['Typology',i.typology],['Prototype',i.prototypeType],['Material',i.material],['Fabrication',i.fabricationMethod],['Phase',i.projectPhase]].map(([a,b])=>`<div class="meta-row"><span>${a}</span><span>${b}</span></div>`).join('')}</div></div>`;
  }
  panel.classList.add('open');
  panel.setAttribute('aria-hidden','false');
  if(i.model)requestAnimationFrame(()=>initModelViewer(i));
}
...
async function initModelViewer(item){
  const host=$('#modelViewport'),status=$('#modelStatus');
  ...
  scene.add(new THREE.HemisphereLight(0xeaf7ff,0x15171a,2.15));
  const key=new THREE.DirectionalLight(0xffffff,3.8);key.position.set(4.5,6,5);key.castShadow=true;scene.add(key);
  const rim=new THREE.DirectionalLight(0xaed7e8,2.2);rim.position.set(-5,2,-4);scene.add(rim);
  const fill=new THREE.DirectionalLight(0xfff0d4,1.25);fill.position.set(0,-2,4);scene.add(fill);

  const floor=new THREE.Mesh(new THREE.CircleGeometry(3.4,96),new THREE.ShadowMaterial({opacity:.22}));
  floor.rotation.x=-Math.PI/2;floor.position.y=-1.28;floor.receiveShadow=true;scene.add(floor);

  const manager=new THREE.LoadingManager();
  manager.onProgress=(url,loaded,total)=>{if(status)status.textContent=`Loading textured object… ${Math.round(loaded/total*100)}%`};
  ...
  modelViewer={scene,camera,renderer,controls,resizeObserver,get animationId(){return animationId},set animationId(v){animationId=v}};
  ...
  try{
    ...
    if(!modelViewer||modelViewer.renderer!==renderer){object.traverse(o=>o.geometry?.dispose?.());return}

    const material=new THREE.MeshStandardMaterial({map:diffuse,normalMap:normal,aoMap:ao,roughness:.9,metalness:.02,side:THREE.DoubleSide});
    ...
    const fit=2.7/Math.max(size.x,size.y,size.z);
    object.scale.setScalar(fit);
    scene.add(object);
    controls.target.set(0,0,0);
    controls.update();
    if(status){status.textContent='Textured OBJ loaded';status.classList.add('loaded');setTimeout(()=>status?.remove(),1100)}
  }catch(error){
    console.error('3D model failed to load',error);
    if(status){status.textContent='Model could not load. Open the site with Live Server and verify the asset paths.';status.classList.add('error')}
  }
...
function renderRelationships(){const visible=relationships.filter(r=>state.filters.has(r.category));$('#relationshipGraph').innerHTML=visible.map(r=>{const a=archiveItems.find(i=>i.id===r.source),b=archiveItems.find(i=>i.id===r.target);return `<article class="relationship-card" data-id="${a.id}"><div class="category">${r.category}</div><h3>${a.title} ↔ ${b.title}</h3><p>${r.description}</p><div class="strength"><span style="width:${r.strength*100}%"></span></div></article>`}).join('')||'<p>No relationship categories selected.</p>';$$('.relationship-card').forEach(c=>c.onclick=()=>{focusItem(c.dataset.id)})}
function renderTimeline(){const years=[...new Set(archiveItems.map(i=>i.year))].sort((a,b)=>b-a);$('#timelineList').innerHTML=years.map(y=>`<section class="year-group"><div class="year-label">${y}</div><div class="year-items">${archiveItems.filter(i=>i.year===y).map(i=>`<article class="timeline-item" data-id="${i.id}"><small>${i.archiveCode}</small><h3>${i.title}</h3><p>${i.location} · ${i.material}</p></article>`).join('')}</div></section>`).join('');$$('.timeline-item').forEach(c=>c.onclick=()=>{focusItem(c.dataset.id)})}
...
function renderSearch(q){q=q.toLowerCase().trim();const res=archiveItems.filter(i=>!q||[i.title,i.material,i.location,i.typology,i.archiveCode].join(' ').toLowerCase().includes(q)).slice(0,20);$('#searchResults').innerHTML=res.map(i=>`<div class="search-result" data-id="${i.id}"><small>${i.archiveCode}</small><strong>${i.title}</strong><span>${i.year} · ${i.location}</span></div>`).join('');$$('.search-result').forEach(r=>r.onclick=()=>{closeSearch();focusItem(r.dataset.id)})}
